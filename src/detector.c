    ...
    while (get_current_iteration(net) < net.max_batches) {
        ...
        */
        ...
        if (calc_map) {
            ...
            if (avg_loss > 0) printf("\n Last avg loss = %f, best = %f ", avg_loss, best_avg_loss);
        }
        ...
        if (calc_map && (iteration >= next_map_calc || iteration == net.max_batches)) {
            ...
            if (mean_average_precision > best_map) {
                ...
                printf("New best mAP!\n");
                char buff[256];
                sprintf(buff, "%s/%s_best_map.weights", backup_directory, base);
                save_weights(net, buff);
            }
            if (mean_average_precision == best_map && avg_loss < best_avg_loss) {
                best_avg_loss = avg_loss;
                printf("New best Average Loss!\n");
                char buff[256];
                sprintf(buff, "%s/%s_best_avg_loss_and_map.weights", backup_directory, base);
                save_weights(net, buff);
            }
        ...
        if (iteration % 100 == 0) {
        //if ((iteration >= (iter_save + 10000) || iteration % 10000 == 0) ||            (iteration >= (iter_save + 1000) || iteration % 1000 == 0) && net.max_batches < 10000) {
            iter_save = iteration;
#ifdef GPU
